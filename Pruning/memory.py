...
class Memory_Unit(Module):
    def __init__(self, nums, dim):
        ...
        self.register_parameter('weight_orig', self.memory_block)
        self.register_buffer('weight_mask', torch.ones_like(self.memory_block))
        self.sig = nn.Sigmoid()
    ...
    def forward(self, data):  ####data size---> B,T,D       K,V size--->K,D
        masked_memory = self.memory_block * self.weight_mask
        attention = self.sig(torch.einsum('btd,kd->btk', data, masked_memory) / (self.dim**0.5))  #### Att---> B,T,K
        temporal_att = torch.topk(attention, self.nums//16+1, dim = -1)[0].mean(-1)
        augment = torch.einsum('btk,kd->btd', attention, masked_memory)                   #### feature_aug B,T,D
